...
def load_model():
    """Load Scaler, LabelEncoder, and RandomForestClassifier models."""
    global _scaler, _encoder, _classifier, _explainer
    ...
    if os.path.exists(classifier_path):
        ...
        logger.info("✅ RandomForestClassifier loaded")
...
def _compute_heuristic_score(feature_vector: np.ndarray) -> Dict:
    ...
    return {
        ...
        # Compatibility keys
        "predicted_class": threat_level,
        "threat_probability": 60.0
    }


def predict_threat(feature_vector: np.ndarray) -> Dict:
    """
    Run inference on a 20-dim feature vector using standard classification.
    """
    ...
    probs = _classifier.predict_proba(scaled)[0]  # Array of 5 class probabilities
    ...
    normal_idx = 4  # Default baseline class order mapping
    if _encoder is not None:
        try:
            normal_idx = list(_encoder.classes_).index("Normal")
        except ValueError:
            pass

    prob_normal = probs[normal_idx]
    
    # 2. Behavioral Anomaly Score based on deviation from "Normal" behavior
    # Map (1.0 - Normal probability) to [0, 100] deviation score
    anomaly_score = float(round((1.0 - prob_normal) * 100.0, 2))
    ...
    # Map class order to risk scores
    class_score_map = {}
    if _encoder is not None:
    ...
    classifier_risk_score = float(sum(probs[i] * class_score_map.get(i, 0.0) for i in range(len(probs))))

    # Final Risk Score = Classifier Risk Score
    final_score = float(round(classifier_risk_score, 2))
    ...
    # 3. SHAP and Analytical Explainability
    shap_vals = None
    ...
    # 4. Natural Language SHAP Explanation & Actions
    reasons = []
    actions = []
    ...
    is_insider = threat_level in ["High Risk", "Critical Risk"] or final_score >= 50.0
    ...
    return {
        ...
        "isolation_forest_score": float(round(anomaly_score, 2)),
        "isolation_forest_level": anomaly_level,
        ...
        "threat_probability": float(round(confidence * 100.0, 1))
    }


def predict_employee(db, employee_id: int, days: int = 30, sync_risk_score: bool = True) -> Dict:
    """Full prediction pipeline: extract features → predict → optionally sync RiskScore."""
    ...
    # Auto-sync result with RiskScore DB table (only when called from single-employee predict)
    if sync_risk_score:
        try:
            from app.models import RiskScore, RiskCategory
            cat_map = {
            ...
            category = cat_map.get(result["threat_level"], RiskCategory.low)
            explanation = {
                "top_factors": [
                ...
                "insider_status": result["insider_status"],
            ...
            if rs:
                ...
                rs.isolation_forest_score = result["isolation_forest_score"]
                ...
                rs = RiskScore(
                    ...
                    privilege_misuse_score=float(result["feature_values"].get("privilege_escalation", 0.0) * 20),
                    data_access_violation_score=float(result["feature_values"].get("data_transfer_size", 0.0) / 10),
                    access_pattern_score=float(result["feature_values"].get("usb_usage", 0.0) * 15),
                    historical_security_score=result["confidence"],
                    ...
                    xgboost_probability=result["confidence"] / 100.0,
                    score_date=datetime.now(timezone.utc)
                )
                db.add(rs)
            db.commit()
        except Exception as e:
            logger.warning(f"Failed to sync RiskScore for employee {employee_id}: {e}")
...
def is_model_loaded() -> bool:
    ...
    return _scaler is not None and _classifier is not None
