...
def train(
    ...
    # We fit it on normal features as standard practice
    X_normal = X_train[y_train == encoder.transform(["Normal"])[0]]
    if len(X_normal) < 5:
        X_normal = X_train  # Fallback if normal class is too small
    ...
    # 2. Train XGBoost Classifier (supervised risk prediction)
    # Configure XGBoost multiclass model with better hyperparameters for class diversity
    ...
    logger.info(f"✅ Saved Isolation Forest → {iforest_path}")
    logger.info(f"✅ Saved XGBoost Classifier → {xgboost_path}")
